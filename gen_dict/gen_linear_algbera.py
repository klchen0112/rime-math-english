...
en_phrase_set = set()
en_word_set = set()
with open("linear_algebra.txt", "r") as src_file:
    for line in src_file.readlines():
        line = line.strip()
        if len(line) == 0:
            continue
        line = line.split(' ')
        zh_word, en_word = line[-1], line[:-1]

        en_word = ' '.join(en_word)
        zh_phrase_set.add(zh_word)
        en_phrase_set.add(en_word)
with open("../linear_albera.extended/linear_algebra_zh.dict.yaml", "w") as dstZH:
    dstZH.write(
        "---\nname: linear_algebra_zh\nversion: \"2021.10.13\"\nsort: by_weight\nuse_preset_vocabulary: false\n...\n\n")
...
with open("../linear_albera.extended/linear_algebra_en_phrase.dict.yaml", "w") as dstENPhrase:
    dstENPhrase.write(
        "---\nname: linear_algebra_en_phrase\nversion: \"2021.10.13\"\nsort: by_weight\nuse_preset_vocabulary: false\n...\n\n")
...
with open("../linear_albera.extended/linear_algebra_en_word.dict.yaml", "w") as dstENWord:
    dstENWord.write(
